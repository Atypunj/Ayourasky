...
# Load fragrance links once
try:
    with open("fragrance_links.json", "r", encoding="utf-8") as f:
        FRAGRANCE_LINKS = json.load(f)
except Exception:
    FRAGRANCE_LINKS = {}

def make_fragrance_link(name: str) -> str:
    """Wrap fragrance name in an <a> if a URL exists in fragrance_links.json."""
    if not name:
        return name
    url = FRAGRANCE_LINKS.get(name)
...
def get_fragrance_for_weak_planet(weak_planet, json_path="weak_planet_fragrance.json"):
    ...
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            mapping = json.load(f)
        options = mapping.get(weak_planet, [])
        return random.choice(options) if options else "No fragrance found for this planet."
...
if request.method == "POST":
    ...
    if not latitude or not longitude:
        return render_template(
            "blessings.html",
            result={"error": "Location not found. Please select your city and state from suggestions."},
        )
    try:
        lat = float(latitude)
        lon = float(longitude)
        ...
        return render_template(
            "blessings.html",
            result={"error": "Invalid location format. Please re-select the suggested location."},
        )
    ...
    # --- Charts & calculations (preserve existing logic as in codebase) ---
    ...
    # Weak/current element (from current time)
    ...
    result = {
        ...
        "moon_sign": moon.sign, "sun_sign": sun.sign, "nakshatra": nakshatra,
        "life_path": life_path, "life_path_fomo": life_path_fomo,
        "birth_element": birth_element, "current_element": current_element, "derived_element": derived_element,
        "rashi_lord": rashi_lord,
        "mahadasha": mahadasha_lord, "mahadasha_start": mahadasha_start_str, "mahadasha_end": mahadasha_end_str,
        "antardasha": antardasha_lord, "antardasha_start": antardasha_start_str, "antardasha_end": antardasha_end_str,
        "remedies": remedies,
    }

    result["intro_paragraph"] = generate_intro({
        "name": name, "place": place, "dob": dob, "tob": tob,
        "ascendant": asc.sign, "moon_sign": moon.sign, "sun_sign": sun.sign,
        "birth_element": birth_element, "current_element": current_element, "derived_element": derived_element
    }, strongest_planet)

    result["merged_elemental_html"] = get_merged_elemental_profile(birth_element)
    hybrid_profile = get_hybrid_elemental_profile(birth_element)
    ...
    result["random_elemental_insight"] = random.choice(result["elemental_insights"]) if result["elemental_insights"] else None
    ...
    # Clickable fragrance links for mahadasha & antardasha
    maha_frag = make_fragrance_link(get_fragrance_for_weak_planet(result['mahadasha']))
    anta_frag = make_fragrance_link(get_fragrance_for_weak_planet(result['antardasha']))
