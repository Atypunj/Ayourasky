...
def _linkify(fr_name: str) -> str:
    if not isinstance(fr_name, str) or not fr_name.strip():
        return fr_name
    if '<a ' in fr_name:
        return fr_name
    url = FRAGRANCE_LINKS.get(fr_name)
    if not url:
        norm = fr_name.replace("'", "").strip().lower()
        for k, v in FRAGRANCE_LINKS.items():
            if k.replace("'", "").strip().lower() == norm:
                url = v
                break
    if url:
        return f'<a href="{url}" target="_blank" rel="noopener">{fr_name}</a>'
    return fr_name




# --- Real → Inspired name normalization (final mapping) ---
INSPIRED_NAME_MAP = {
    # Core inspired mappings
    "Agarwood Oil": "Oud Nirvana",
    "Cedarwood Oil": "Kalpavriksha Moksh",
    "Frankincense Oil": "Sacred Dhupa",
    "Jasmine Oil": "Vrindavan Pushpa",
    "Camphor Oil": "Aarti Aroma",
    "Kesar Oil": "Sage's Blessing",
    "Lavender Oil": "Shanti Chaitanya",
    "Loban Oil": "Pavitra Dhuna",
    "Lotus Oil": "Kamal Sparsh",
    "Myrrh Oil": "Tapasya Nectar",
    "Patchouli Oil": "Bhakti Kusum",
    "Rose Oil": "Radha Rasa",
    "Sandalwood Oil": "Sandalwood Nirvana",
    "Tagar Oil": "Tapovan Sadhana",
    "Vetiver Oil": "Bhoomi Aura",
    # Variants normalization → inspired keys used in your JSON
    "Sages Blessing": "Sage's Blessing",
    "Prithvi Smaran": "Prithvismaran",
    "Vayutattva": "Vāyutattva",
    "Akasha Jyoti": "Ākāsha Jyoti",
    "Jalasakti": "Jalasākti",
}

def normalize_to_inspired(name: str) -> str:
    if not isinstance(name, str):
        return name
    name = name.strip()
    return INSPIRED_NAME_MAP.get(name, name)

def _extract_inner_paren(text_value: str) -> str:
    """If string contains (...) return the inner content; else return as-is."""
    if not isinstance(text_value, str):
        return text_value
    if '(' in text_value and ')' in text_value:
        try:
            start = text_value.index('(') + 1
            end = text_value.rindex(')')
            if end > start:
...
SOUL_PATH  = BASE / "static" / "data" / "soul_scent_logic.json"
LINKS_PATH = BASE / "static" / "data" / "fragrance_links.fixed.json"
...
def _load_indexes():
    ...
    syn           = soul.get("synonyms", {})
    frag_bullets  = soul.get("fragrance_bullets", {})
    dasha_bullets = soul.get("dasha_bullets", {})

    return {
        "syn":           { _norm(k): _norm(v) for k, v in syn.items() },
        "frag_bullets":  { _norm(k): v for k, v in frag_bullets.items() },
        "dasha_bullets": dasha_bullets,
        "links":         { _norm(k): v for k, v in links.items() },
    }

def _resolve_name(name: str) -> str:
    idx = _load_indexes()
    n = _norm(name)
    return idx["syn"].get(n, n)

def bullets_for(dasha: str, frag_name: str):
    idx = _load_indexes()
    key = _resolve_name(frag_name)
    return idx["frag_bullets"].get(key) or idx["dasha_bullets"].get(dasha, [])

def link_for(frag_name: str):
    idx = _load_indexes()
    return idx["links"].get(_resolve_name(frag_name))

# make helpers available in all templates
@app.context_processor
def inject_helpers():
    return dict(bullets_for=bullets_for, link_for=link_for)
...
# [Duplicate Flask init and dynamic JSON loads removed — using the earlier initialized app & loaded JSONs]
...
def index():
    if request.method == "POST":
        ...
        def _jd_from_dt(_dt):
            return swe.julday(_dt.year, _dt.month, _dt.day) + (_dt.hour + _dt.minute/60 + _dt.second/3600) / 24.0

        try:
            # 1) Find start of current lunar bucket
            _now_dt = datetime.datetime.utcnow()
            _now_jd = _jd_from_dt(_now_dt)
            _bucket_index = int((_now_jd - ANCHOR_JD) // SYNODIC_DAYS)
            _bucket_start_jd = ANCHOR_JD + _bucket_index * SYNODIC_DAYS

            # 2) Build a chart at *bucket start* for user's place, then derive element once
            _by, _bm, _bd, _but = swe.revjul(_bucket_start_jd)  # returns (y, m, d, ut)
            _bucket_dt = Datetime(f"{_by}/{_bm:02d}/{_bd:02d}", f"{int(_but):02d}:{int((_but%1)*60):02d}", "+05:30")
            _bucket_chart = Chart(_bucket_dt, pos)
            _bucket_element, _ = calculate_dominant_element(_bucket_chart, rahu_sign, ketu_sign)

            # 3) Pin the current_element to this deterministic bucket value
            current_element = _bucket_element
        except Exception:
            # Fail-safe: keep the previously computed transient value
            pass
 ...
 "moon_sign": moon.sign,
    ...
    # 🧠 DEBUG: Pause before weak planet fragrance
        ...
        result["fragrance_inspired_name"] = normalize_to_inspired(result.get("fragrance_inspired_name", ""))
        result["fragrance_inspired_name_linked"] = _linkify(result.get("fragrance_inspired_name", ""))
        
        # Safety: if not populated, try again
        if not result.get('fragrance_inspired_name_linked'):
            result['fragrance_inspired_name_linked'] = _linkify(result.get('fragrance_inspired_name', ''))
